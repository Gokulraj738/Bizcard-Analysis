...
def create_table_if_not_exists(cursor):
    cursor.execute('''CREATE TABLE IF NOT EXISTS bizcard_details (
        NAME VARCHAR(255), DESIGNATION VARCHAR(255), COMPANY_NAME VARCHAR(255),
        CONTACT VARCHAR(255), EMAIL VARCHAR(255), WEBSITE VARCHAR(255), ADDRESS TEXT,
...
if select == "Home":
  ...
  if img is not None:
    ...
    if button_1:
      ...
      data = df.values.tolist()[0] + [image_data]
  ...
  if method == "Preview":
    ...
    if button_3:      
      ...
      datas = df_4.values.tolist()[0] + [image_data]
